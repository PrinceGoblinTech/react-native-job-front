...
function BidBoardList({ navigation, currentState }) {
  const boardList = [
    {
      ...
      boardList.map((item, i) => (
        <View key={i} style = {[globalStyles.h163, globalStyles.mb12, globalStyles.br16, globalStyles.ml16, globalStyles.mr16, {backgroundColor: '#F5F5F5'}]}>
          <TouchableHighlight style = {[globalStyles.br16, globalStyles.h163]}
            activeOpacity = {0.6}
            underlayColor = '#ccc'
            onPress={() => navigation.navigate('bid_details', {state: item.state})}
            >
            <View style = {[globalStyles.h163, globalStyles.pr16, globalStyles.pl16, globalStyles.pt13]}>
              <View style = {globalStyles.rowFlexShow}>
                <Text style = {[globalStyles.fontMont, globalStyles.fs12, globalStyles.lh18, globalStyles.lSpace4, globalStyles.fw600, {color: '#C87D12'}]}>{item.dateLimit} Days</Text>
                <Text style = {[globalStyles.fontMont, globalStyles.fs12, globalStyles.lh18, globalStyles.lSpace4, globalStyles.flex1, globalStyles.fw400, {color: 'rgba(0, 0, 0, 0.38)'}]}> left to Bid</Text>
                <View style = {[globalStyles.mt_5, globalStyles.w52, globalStyles.h26, globalStyles.br4, globalStyles.algCtr, globalStyles.justCtr, {backgroundColor: '#C4F1D2'}]}>
                  <Text style = {[globalStyles.fontMont, globalStyles.fs12, globalStyles.lh18, globalStyles.lSpace4, globalStyles.fw400, {color: '#00B53A'}]}>{item.state ? 'Open' : 'Close'}</Text>
                </View>
              </View>
              ...
              <View style = {[globalStyles.rowFlexShow, globalStyles.mt12]}>
                ...
                <Text style = {[globalStyles.fontMont, globalStyles.fs12, globalStyles.lh18, globalStyles.lSpace4, globalStyles.fw400, {color: 'rgba(0, 0, 0, 0.6)'}]}>{item.bidCnt} Bidders</Text>
              </View>
            </View>
          </TouchableHighlight>
        </View>
